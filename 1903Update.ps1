...
If ($spaceAvailable -lt 15) {
  Write-Output "You only have a total of $spaceAvailable GBs available, this upgrade needs 10GBs or more to complete successfully: $dateTime" | Out-File $logFile -Append
  Break
}
#region checkDisk
...
If ($rbCheck1 -ne $Null -or $rbCheck2 -ne $Null -or $rbCheck3 -ne $Null){
    Write-Output "This system is pending a reboot, unable to proceed. Please restart your computer and try again.: $dateTime" | Out-File $logFile -Append
    Break
} Else {
    Write-Output "Automation has verified there is no reboot pending - $dateTime" | Out-File $logFile -Append
}

If ([System.Environment]::OSVersion.Version.Major -ne 10) {
  Write-Output "Your version of Windows does not support the 1903 upgrade. Exiting script. $dateTime" | Out-File $logFile -Append
  Break
}

$osBuild = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -Name ReleaseId).ReleaseId
If ($osBuild -ge 1903) {
  Write-Output "Your machine already has this patch installed! Exiting script. $dateTime" | Out-File $logFile -Append
  Break
}
...
Try {
  ...
  Write-Error "Failed to create the following folder: $1903Dir - $dateTime" | Out-File $logFile -Append
  Return
}
...
If ($checkZip) {
  ...
  If ($servFile -gt (Get-Item $1903Zip).Length) {
    ...
    If (!$checkFile) {
      $status = 'Download'
      Write-Output "The existing installation files for the 1903 update were incomplete or corrupt. Deleted existing files and started a new download. - $dateTime" | Out-File $logFile -Append
    }
    Else {
      Write-Output "Failed to delete the installation files for 1903. Exiting script. $dateTime" | Out-File $logFile -Append
      Break
    }
  }
  Else {
    Write-Output "Verified the installation package downloaded successfully! - $dateTime" | Out-File $logFile -Append
    $status = 'Unzip'
  }
}
Else {
  $status = 'Download'
  Write-Output "The required files to install the 1903 update are not present, downloading required files now. Keep in mind this update is VERY large, and may take 30min+ to download (depending on your connection speed). - $dateTime" | Out-File $logFile -Append
}
...
If ($status -eq 'Download') {
  Try {
    ...
    If ($servFile -gt (Get-Item $1903Zip).Length) {
      Write-Error "The downloaded size of $1903Zip does not match the server version, unable to install the update. - $dateTime" | Out-File $logFile -Append
    } Else {
      Write-Output "Successfully downloaded the 1903 update! - $dateTime" | Out-File $logFile -Append
      $status = 'Unzip'
    }
  } Catch {
    Write-Error "Encountered a problem when trying to download the Windows 10 1903 ISO - $dateTime" | Out-File $logFile -Append
  }
}
...
Try {
  If ($status -eq 'Unzip') {
    ...
    Write-Output "Unpacking 1903 installation files...this will take awhile. - $dateTime" | Out-File $logFile -Append
    &$7zip x $1903Zip -o"$1903Dir" -y | Out-Null
    Write-Output "Unpacking complete! Beginning 1903 upgrade installation. - $dateTime" | Out-File $logFile -Append
    $status = 'Install'
  }
  ...
  If ($status -eq 'Install') {
    ...
    $localFolder= (Get-Location).path
    $process = Start-Process -FilePath "$1903Dir\Pro$osVer.1903\setup.exe" -ArgumentList "/auto upgrade /quiet /Compat IgnoreWarning /DynamicUpdate enable /copylogs $windowslogs /migratedrivers all" -PassThru
    For($i = 0; $i -le 100; $i = ($i + 1) % 100) {
      Write-Progress -Activity "Installer" -PercentComplete $i -Status "Installing"
      Start-Sleep -Milliseconds 100
      If ($process.HasExited) {
        Write-Progress -Activity "Installer" -Completed
        Write-Output "Installation has completed, deleting setup files - $dateTime" | Out-File $logFile -Append
        Remove-Tree -Path $1903Dir
        Write-Output "1903 upgrade process complete! - $dateTime"
        Break
      }
    }
  } Else {
  ...
  Write-Error "Setup ran into an issue while attempting to install the 1903 upgrade. - $dateTime"
}
